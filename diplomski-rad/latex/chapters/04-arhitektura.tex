...
\begin{longtable}{@{}|>{\small\raggedright\arraybackslash}p{0.232\textwidth}|>{\small\raggedright\arraybackslash}p{0.232\textwidth}|>{\small\raggedright\arraybackslash}p{0.232\textwidth}|>{\small\raggedright\arraybackslash}p{0.232\textwidth}|@{}}
\caption{Servisi definisani u \texttt{docker-compose.yml}}\label{tab:4-1}\\
\hline
\rowcolor{black!12}
\textbf{Servis} & \textbf{Slika} & \textbf{Uloga} & \textbf{Port\newline (host)} \\
\hline
\endhead
\texttt{postgres} & \texttt{postgis/\allowbreak postgis:\allowbreak 16-\allowbreak 3.4-\allowbreak alpine} & Relaciona baza + PostGIS ekstenzija & 5433 → 5432 \\
\hline
\texttt{rabbitmq} & \texttt{rabbitmq:\allowbreak 3-\allowbreak management-\allowbreak alpine} & Posrednik poruka (AMQP) + web konzola & 5672, 15672 \\
\hline
\texttt{valhalla} & \texttt{ghcr.\allowbreak io/\allowbreak valhalla/\allowbreak valhalla:\allowbreak latest} & HTTP routing servis (\texttt{valhalla\_\allowbreak service}) & 8002 \\
\hline
\texttt{valhalla-\allowbreak build} & \texttt{ghcr.\allowbreak io/\allowbreak valhalla/\allowbreak valhalla:\allowbreak latest} & Jednokratna izgradnja grafa iz OSM ekstrakta (profil \texttt{build}, ne pokreće se pri običnom \texttt{up}) & — \\
\hline
